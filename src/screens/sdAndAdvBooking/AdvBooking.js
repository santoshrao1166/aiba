...
function AdvBooking({navigation}) {
...
const BookingDetailsHeader = () => {
  return (
    <>
      {/* 
Time	Month	Year	Page	Amount	Invoice */}
      <View
        style={{
...
function BookingDetails({booking, history, getBookings, navigation, other}) {
  ...
  const getInvoice = () => {
    ...
    if (invNum) {
      ...
      let invUrl = `${serverEndPoint}user/advert_booking_invoice/${invBase64}`;
  ...
  return (
    <View
      ...
      <View style={{width: 100}}>
        <Text style={{textAlign: 'center'}}>{booking?.time}</Text>
      </View>
      <View style={{width: 100}}>
