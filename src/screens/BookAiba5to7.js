...
////////////////////////////////////////////////////////////////
function BookAiba5to7({ navigation }) {
  ...
  const [timeSlots, settimeSlots] = useState(nonMembersTimeSlot);
  ...
  function getNextDates2(startDate) {
    var aryDates = [];
    var lastDayOfMonth = new Date(new Date(startDate.getFullYear(), startDate.getMonth()+2, 0).setDate(new Date(startDate.getFullYear(), startDate.getMonth()+2, 0).getDate() + 1));
    aryDates.push({
  ...
  const fetchGroupData = async () => {
    axiosGet(
      "/user/user_odd_groups?type=nonmembers",
      (data) => {
        setallGroups(data);
      },
      (error) => {},
      ...
      orderCreation => {
        ...
        RazorpayCheckout.open(options)
          .then(async response => {
            ...
            navigation.navigate('Multi Bookings AIBA 5to7');
          })
          .catch(async(error) => {
            clearUserCart();
      ...
      <View style={{ marginHorizontal: width * 0.05, flex: 1 }}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          ...
          >
            ...
            <TouchableOpacity
              onPress={() => navigation.navigate('Multi Bookings AIBA 5to7')}
              style={{
                padding: 10,
                marginTop: 20,
          ...
          <>
                    ...
                    if (group?.id == val) {
                      ...
                      settimeSlots(
                        group?.group_type == "even"
                          ...
                          : group?.group_type == "sd"
                          ? sdGroupTimeSlot
                          : group?.group_type == "nonmembers"?nonMembersTimeSlot:oddGroupTimeSlot
                      );
                    }
                  });
                }}
                    ...
                    <Picker.Item
                      ...
                      label={`${group?.name} ( ${
                        group?.group_type == "even"
                          ...
                          : group?.group_type == "sd"
                          ? "Shoppers Darbar"
                          : group?.group_type == "nonmembers" ? "NonMembers": "Odd Hours"
                      }  )`}
                      value={group?.id}
                    />
                  ))
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {nextSevenDays?.map((day, i) => (
                <SlotDate
                  key={i}
                  selected={selectedDate?.date == day?.date && selectedDate?.month == day?.month ? true : false}
                  wholeDateObj={day}
            ...
            {isLoading ? (
              ...
              errorInBookingSlots ? (
                ...
                timeSlots?.map((slot, i) => {
                  if((selectedDate.date == new Date().getDate()) && (selectedDate.month == new Date().getMonth()+1))
                  {
                  if(slot.startTime > new Date().getHours()){
                    return <TimeSlotBar
                    key={i}
                    date={selectedDate}
                    plan={selectedPlan}
